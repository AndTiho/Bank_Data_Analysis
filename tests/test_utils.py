...
from src.utils import (PATH_TO_JSON, greetings, json_settings_for_currency,
                       json_settings_for_stocks, excel_to_df, PATH_TO_EXCEL, filter_by_date, filter_by_date_three_month,
                       total_spent, top_transactions, group_by, write_to_file, get_currency, get_stocks_data)
...
def test_get_stocks_data_correct_work():
    with patch('urllib.request.urlopen') as mock_urlopen:
        ...
        mock_response_googl.read.return_value = json.dumps([{
        "symbol": "GOOGL",
        "price": 2800.0
        }]).encode('utf-8')
        ...
        assert get_stocks_data(['AAPL', 'GOOGL']) == {
            'AAPL': 150.0,
            'GOOGL': 2800.0
        }
...
@pytest.mark.parametrize('hour, expected_result', [(1, 'Доброй ночи'), (5, 'Доброе утро'),(10, 'Доброе утро'),
                                                   (11, 'Добрый день'), (15, 'Добрый день'),
                                                   (16, 'Добрый вечер'), (22, 'Добрый вечер'), (23, 'Доброй ночи')])
