...
def main()-> str:
    ...
    # Здесь отрабатываем функционал модуля services.py
        ...
        while True:
            ...
            try:
                ...
                print("Неверный формат даты!")
                print(
                    "ПОЖАЛУЙСТА!!!!! введите дату в формате YYYY-MM"
    ...
    # Тут работаем с функционалом из reports.py
        ...
        while category not in group_by(df):
            print(
                f"Выберете категорию из списка {group_by(df)}\n"
        ...
        time.sleep(1)
        print(
            'Напоминаем, что по заданию функция декорирована записью("w") результата в results.txt '
        )
        time.sleep(3)
        return result
